...
core.register_craftitem("foods:spawn", {
    description = "Spawn 3D Model",
    inventory_image = "mymod_inventory.png",
    ...
    on_use = function(itemstack, user, pointed_thing)
        ...
        -- Set the raycast to search in front of the player (e.g., up to 5 blocks)
        local range = 5
        local p1 = vector.add(player_pos, {x = 0, y = user:get_properties().eye_height, z = 0})
        local p2 = vector.add(p1, vector.multiply(dir, range))
        ...
        if hit and hit.type == "node" then
            -- Spawn directly on the surface the player is pointing at
            spawn_pos = hit.above
        else
            -- If pointing at the sky or too far, spawn at max range
            spawn_pos = p2
        end

        -- Spawn the 3D entity
        core.add_entity(spawn_pos, "foods:my_3d_model")
        
        -- Consume one item on use
        itemstack:take_item()
        return itemstack
    end,
})

core.register_entity("foods:my_3d_model", {
    initial_properties = {
        visual = "mesh",
        mesh = "raccoon.gltf", -- Your 3D mesh file
        textures = {"raccoon.png"}, -- Model texture
        visual_size = {x = 5, y = 5},
        collisionbox = {-0.5, -0.5, -0.5, 0.5, 0.5, 0.5},
    },
    
    -- Optional: Make the entity do something or remove it upon being punched/right-clicked
